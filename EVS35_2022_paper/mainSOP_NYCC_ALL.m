...
load('A004_DYN_P30_NYCC.mat'); % loads data


adap = [0 0 0 0 1];
ru = [1e-6 1e-7 1e-8 1e-9 1e-1];
saveName1 = 'results/SOP_NYCC_';
...
% Covariance values
SigmaX0 = diag([1e-6 1e-3 .5e-5 1e-2 1e-1]); % uncertainty of initial state
SigmaV = diag([1e-2 2e-4]); % Uncertainty of voltage sensor, output equation
SigmaW = diag([1e-2 1e-2]); % Uncertainty of current sensor, state equation
spkfData = initSPKF(SigmaX0,SigmaV,SigmaW,model,SOC0,Tc0,Ts0);

%% SOP limits
mpcData.const.z_max = 0.95;     % Max SOC
mpcData.const.z_min = 0.1;     % Min SOC
mpcData.const.du_max =  50;    % Max control increment (Used by adaptive input weighting)
